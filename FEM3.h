...
double FEM<dim>::C(unsigned int i, unsigned int j, unsigned int k, unsigned int l) {
    ...
    static const double E = 2 * pow(10, 11), //EDIT
                        nu = 0.3; //EDIT
...
void FEM<dim>::define_boundary_conds() {
    ...
    for (int globalDOF = 0; globalDOF < totalDOFs; globalDOF++) {

        if (dofLocation[globalDOF][2] == 0.0) {
...
void FEM<dim>::assemble_system() {
    ...
    for (; elem != endc; ++elem) {
                        ...
                        for (unsigned int k = 0; k < dim; k++) {
                            for (unsigned int j = 0; j < dim; j++) {
                                for (unsigned int l = 0; l < dim; l++) {
                                    ...
                                    Klocal[3 * A + i][3 * B + k] += fe_values.shape_grad(dim * A + i, q)[j] \
                                                                    *C(i, j, k, l) \
        ...
        for (unsigned int i = 0; i < dofs_per_elem; i++) {
            ...
            unsigned int A_global = local_dof_indices[i];
            F[A_global] += Flocal[i];
            for (unsigned int j = 0; j < dofs_per_elem; j++) {
                //EDIT - Assemble K from Klocal (you can look at HW2)
                unsigned int B_global = local_dof_indices[j];
                K.add(A_global, B_global, Klocal[i][j]);
            }
        }
    }
